...
module ``about the stock example`` =
    ...
    let parseRow (indexDate, indexPrice1, indexPrice2, x:System.String[]) = 
        (System.DateTime.Parse(x.[indexDate]), System.Decimal.Parse(x.[indexPrice1]), System.Decimal.Parse(x.[indexPrice2]))

    let getVariance(date:System.DateTime, low:System.Decimal, high:System.Decimal) = 
        System.Math.Abs(high - low)

//    let parseHeaders(headerRow, columnHeadings) =
//        let columnHeadings = [splitCommas(headerRow)]
//        [for value in columnHeadings do yield Array.findIndex(a=>a == value)]



    [<Koan>]
    let MakeAStringArrayATuple() =        
        let foo = [|"2012-03-30"; "32.40"; "32.41"; "32.04"; "32.26"; "31749400"; "32.26"|]
        
        let date, openValue, closeValue = parseRow(0, 1, 4, foo)

        AssertEquality 32.40m openValue
        AssertEquality 32.26m closeValue


    [<Koan>]
    let VaianceWithPriceIncrease() =
        let date = System.DateTime.Parse "2012-12-30"
        let openValue = 32.04m
        let closeValue = 32.41m
        
        let vaiance = getVariance(date, openValue, closeValue)

        AssertEquality 0.37m vaiance

    [<Koan>]
    let VaianceWithPriceDecrease() =
        let date = System.DateTime.Parse "2012-12-30"
        let openValue = 32.41m
        let closeValue = 32.04m
...
//    [<Koan>]
//    let ParseHeaderRowReturnsRowIndex() = 
//        let headerRow = "Date,Open,High,Low,Close,Volume,Adj Close"        
//        let result = parseHeaders headerRow ["Date", "Low", "High"]
//        result
    ...
    let YouGotTheAnswerCorrect() =
        ...
        let parseRowOpenClose row = parseRow(0, 1, 4, row)
        

        let date, low, high =
            stockData.Tail        
            |> Seq.map splitCommas
            |> Seq.map parseRowOpenClose
